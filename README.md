...
- **작업대** — 홈 상단 "작업대" 버튼을 누르면 홈 위로 떠 있는 창이 열려, 도구를 최대 5칸까지 나란히 두고 동시에 사용할 수 있습니다. 분할 수를 직접 고르고, 칸 사이 경계를 드래그해 너비를 조절하며, 도구를 다른 칸으로 끌어 옮길 수 있습니다(옮겨도 입력한 내용은 그대로 유지됩니다). 도구는 검색해서 선택합니다.
...
- 체지방률 계산기 (`/body-fat/`)
- 1RM 최대중량 계산기 (`/one-rep-max/`)
- 자녀 키 예측 계산기 (`/child-height/`)
...
- 주택청약 가점 계산기 (`/housing-subscription/`)
...
- clip-path 생성기 (`/clip-path-generator/`)
...
- 가짜 트윗(X) 생성기 (`/fake-tweet/`)
- 가짜 인스타 게시물·DM 생성기 (`/fake-instagram/`)
