...
- **디자인 구조 개선 — AdSense 승인 후 빅스윕 때 진행하기로 사용자와 합의(2026-06-13).** 디자인 리뷰 후속. **이미 선반영(리스크 낮아 당겨 적용)**: 아이콘 칩 배경·텍스트 대비(커밋 f96f498), 본문 줄글 폭 제한 해제(3034aea), 홈 카드 그리드 1320px 5열 확장 + 히어로 우측 패널 정렬(972b895). **빅스윕 때 마저 할 잔여 항목**(전 페이지 영향이 커 footer.js 빅스윕과 함께): ① **홈 카드벽 단조로움 — 남은 부분**: 카테고리 섹션 헤더 강화(아이콘+이름+개수 배지, 옅은 배경 밴드), 상단 "인기 도구" 6개 피처링 행, 카테고리별 아이콘 색조 차별화. ② **도구 페이지에서 기능 영역(`.box`) 강조** — 기능 박스를 SEO 텍스트보다 시각적 주인공으로(약간 크게·상단 액센트 보더/옅은 그림자), 결과·빈 상태 디자인 개선. ③ **"테두리 카드" 패턴 과다 차별화** — 사용법은 액센트 숫자 매김, 팁은 가벼운 인라인 리스트, 테두리 카드는 FAQ에만. 실제 개선안은 frontend-design으로 시안을 만들어 비교 후 적용. (승인 전에는 큰 시각 변경 푸시 금지 — 심사 안정성 우선.)
